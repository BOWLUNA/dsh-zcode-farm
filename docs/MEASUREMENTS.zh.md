...
`得分 = 空闲显存 − 队列深度 × 权重`，默认权重 1000。用 `tools/ordering-replay.mjs`
在 `test/fixtures/` 的夹具上重放：

| 夹具 | 权重 0 | 权重 11.9 | 权重 12.1 | 权重 1000 |
| --- | --- | --- | --- | --- |
| 甲 · 2026-09-21 实测 | `gpu-18303` | `gpu-18303` | `gpu-18303` | `gpu-18303` |
| 乙 · 构造的对抗用例 | A | A | B | B |

实测那个夹具在每种权重下都给出同一个答案，因为那天「队列空」和「显存最多」恰好是同一台。
**那次运行区分不了两条规则**，所以它**不是**「队列主导排序」的证据 —— 尽管它看起来像。
两条规则分岔的地方在构造夹具里：交叉点落在权重 12（`20 − w` 与 `8` 相等处），
也就是说默认的 1000 让队列变成**绝对判据**，而不是一个加权的因素。

由 `test/ordering.test.mjs` 守住，8 项检查。
